...
def get_multi_region_boxes(
    ...
    output = (
        output.view(batch * num_anchors, 2 * num_keypoints + 1 + num_classes, h * w)
        .transpose(0, 1)
        .contiguous()
        .view(2 * num_keypoints + 1 + num_classes, batch * num_anchors * h * w)
    )
    grid_x = (
        ...
        for cy in range(h):
            for cx in range(w):
                for i in range(num_anchors):
                    ...
                    if (det_confs[ind] > max_conf) and (
                        cls_confs[ind, correspondingclass] > max_cls_conf
                    ):
                    ...
                    if conf > conf_thresh:
                        ...
                        boxes.append(box)
        if (len(boxes) == 0) or (
            not (correspondingclass in np.array(boxes)[:, 2 * num_keypoints + 2])
        ):
            bcx = list()
            bcy = list()
            for j in range(num_keypoints):
...
def read_truths(lab_path, num_keypoints=9):
    ...
    if os.path.getsize(lab_path):
        ...
        truths = truths.reshape(
            truths.size // num_labels, num_labels
