...
def valid(datacfg, cfgfile, weightfile):
    ...
    corners3D             = utils.get_3D_corners(vertices)
    diam                  = float(data_options['diam'])
    intrinsic_calibration = utils.get_camera_intrinsic(u0, v0, fx, fy) # camera params
    ...
    utils.logging('Testing {}...'.format(name))
    for batch_idx, (data, target) in enumerate(test_loader):
        ...
        all_boxes = utils.get_multi_region_boxes(output, conf_thresh, num_classes, num_keypoints, anchors, num_anchors, int(trgt[0][0]), only_objectness=0)
        t4 = time.time()
            ...
            for k in range(num_gts):
                ...
                for j in range(len(boxes)):
                    if (boxes[j][2*num_keypoints] > best_conf_est) and (boxes[j][2*num_keypoints+2] == int(truths[k][0])):
                        ...
                        box_pr        = boxes[j]
                        match         = utils.corner_confidence(box_gt[:2*num_keypoints], torch.FloatTensor(boxes[j][:2*num_keypoints]))
                ...
                corners2D_gt_corrected = utils.fix_corner_order(corners2D_gt) # Fix the order of corners
                ...
                R_gt, t_gt = utils.pnp(objpoints3D,  corners2D_gt_corrected, K)
                R_pr, t_pr = utils.pnp(objpoints3D,  corners2D_pr, K)

                # Compute pixel error
                Rt_gt        = np.concatenate((R_gt, t_gt), axis=1)
                Rt_pr        = np.concatenate((R_pr, t_pr), axis=1)
                proj_2d_gt   = utils.compute_projection(vertices, Rt_gt, intrinsic_calibration)
                proj_2d_pred = utils.compute_projection(vertices, Rt_pr, intrinsic_calibration)
                proj_corners_gt = np.transpose(utils.compute_projection(corners3D, Rt_gt, intrinsic_calibration))
                proj_corners_pr = np.transpose(utils.compute_projection(corners3D, Rt_pr, intrinsic_calibration))
