...
def fill_truth_detection(
    labpath, w, h, flip, dx, dy, sx, sy, num_keypoints, max_num_gt
):
...
def change_background(img, mask, bg):
    ...
    for c in range(len(imcs)):
        negmask = maskcs[c].point(lambda i: 1 - i / 255)
        posmask = maskcs[c].point(lambda i: i / 255)
        fics[c] = ImageMath.eval(
            "a * c + b * d", a=imcs[c], b=bgcs[c], c=posmask, d=negmask
        ).convert("L")
    out = Image.merge(img.mode, tuple(fics))
...
def shifted_data_augmentation_with_mask(
    img, mask, shape, jitter, hue, saturation, exposure
):
    ow, oh = img.size
...
def superimpose_masks(mask, total_mask):
    ...
    for c in range(len(maskcs)):
        ...
        fics[c] = ImageMath.eval(
            "c + b * d", b=total_maskcs[c], c=posmask, d=negmask
        ).convert("L")
...
def augment_objects(
    ...
    maskpath = (
        imgpath.replace("JPEGImages", "mask")
        .replace("/00", "/")
        .replace(".jpg", ".png")
    )
    ...
    img, mask, flip, dx, dy, sx, sy = shifted_data_augmentation_with_mask(
        img, mask, shape, jitter, hue, saturation, exposure
    )
    label = fill_truth_detection(
        labpath, iw, ih, flip, dx, dy, 1.0 / sx, 1.0 / sy, num_keypoints, max_num_gt
    )
    ...
    for obj in add_objs:
        ...
        while not successful:
            ...
            obj_rand_lab_path = (
                obj_rand_img_path.replace("images", "labels")
                .replace("JPEGImages", "labels")
                .replace(".jpg", ".txt")
                .replace(".png", ".txt")
            )
            ...
            obj_rand_label = fill_truth_detection(
                obj_rand_lab_path,
                iw,
                ih,
                flip,
            ...
            if intersection_ratio < 0.2:
                successful = True
                total_mask = superimpose_masks(
                    obj_rand_mask, total_mask
                )  #  total_mask + obj_rand_mask
                total_masked_img = superimpose_masked_imgs(
                    obj_rand_masked_img, obj_rand_mask, total_masked_img
                )  # total_masked_img + obj_rand_masked_img
...
def load_data_detection(
    imgpath, shape, jitter, hue, saturation, exposure, bgpath, num_keypoints, max_num_gt
):
    ...
    total_masked_img, label, total_mask = augment_objects(
        imgpath,
        objname,
        add_objs,
        shape,
