...
class RegionLoss(nn.Module):
    ...
    def forward(self, output, target, epoch):
        ...
        conf = torch.sigmoid(
            output.index_select(
                2, Variable(torch.cuda.LongTensor([2 * num_keypoints]))
            ).view(nB, nA, nH, nW)
        )
        cls = output.index_select(
            ...
            Variable(
                torch.linspace(
                    2 * num_keypoints + 1, 2 * num_keypoints + 1 + nC - 1, nC
                )
                .long()
                .cuda()
            ),
        ...
        print(
            "%d: nGT %d, recall %d, proposals %d, loss: x %f, y %f, conf %f, total %f"
            % (
                self.seen,
                nGT,
                nCorrect,
