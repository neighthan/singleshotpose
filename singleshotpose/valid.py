...
def valid(datacfg, modelcfg, weightfile):
    ...
    valid_dataset = dataset.listDataset(
        valid_images,
        ...
        for box_pr, target in zip([all_boxes], [target[0]]):
            ...
            for k in range(num_gts):
                ...
                corners2D_gt = np.array(
                    np.reshape(box_gt[:18], [-1, 2]), dtype="float32"
                )
                corners2D_pr = np.array(
                    np.reshape(box_pr[:18], [-1, 2]), dtype="float32"
                )
                ...
                if save:
                    ...
                    np.savetxt(
                        backupdir + "/test/gt/R_" + valid_files[count][-8:-3] + "txt",
                        np.array(R_gt, dtype="float32"),
                    )
                    np.savetxt(
                        backupdir + "/test/gt/t_" + valid_files[count][-8:-3] + "txt",
                        np.array(t_gt, dtype="float32"),
                    )
                    np.savetxt(
                        backupdir + "/test/pr/R_" + valid_files[count][-8:-3] + "txt",
                        np.array(R_pr, dtype="float32"),
                    )
                    np.savetxt(
                        backupdir + "/test/pr/t_" + valid_files[count][-8:-3] + "txt",
                        np.array(t_pr, dtype="float32"),
                    )
                    np.savetxt(
                        backupdir
                        + "/test/gt/corners_"
                        + valid_files[count][-8:-3]
                        + "txt",
    ...
    acc5cm5deg = (
        len(np.where((np.array(errs_trans) <= 0.05) & (np.array(errs_angle) <= 5))[0])
        * 100.0
        / (len(errs_trans) + eps)
    )
    acc3d10 = (
        len(np.where(np.array(errs_3d) <= diam * 0.1)[0]) * 100.0 / (len(errs_3d) + eps)
    )
    acc5cm5deg = (
        len(np.where((np.array(errs_trans) <= 0.05) & (np.array(errs_angle) <= 5))[0])
        * 100.0
    ...
    logging(
        "   Acc using 10% threshold - {} vx 3D Transformation = {:.2f}%".format(
            diam * 0.1, acc3d10
        )
    )
    ...
    logging(
        ...
        % (
            testing_error_trans / nts,
            testing_error_angle / nts,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--modelcfg", type=str, default="cfg/yolo-pose.cfg"
    )  # network config
    parser.add_argument(
        "--weightfile", type=str, default="backup/ape/model_backup.weights"
