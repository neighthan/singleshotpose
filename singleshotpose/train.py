...
def test(epoch, niter):
        ...
        for box_pr, target in zip([all_boxes], [target[0]]):
            ...
            for k in range(num_gts):
                ...
                corners2D_gt = np.array(
                    np.reshape(box_gt[: num_keypoints * 2], [num_keypoints, 2]),
                    dtype="float32",
                )
                corners2D_pr = np.array(
                    np.reshape(box_pr[: num_keypoints * 2], [num_keypoints, 2]),
                    dtype="float32",
                )
    ...
    acc3d = (
        len(np.where(np.array(errs_3d) <= vx_threshold)[0])
        * 100.0
        / (len(errs_3d) + eps)
    )
    acc5cm5deg = (
    ...
    logging(
        "   Acc using {} vx 3D Transformation = {:.2f}%".format(vx_threshold, acc3d)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--initweightfile", type=str, default="cfg/darknet19_448.conv.23"
    )  # imagenet initialized weights
    parser.add_argument(
        "--pretrain_num_epochs", type=int, default=15
    )  # how many epoch to pretrain
